# 测试方案 M5: Learn Two-Round 检索

**日期:** 2026-04-23
**前置文档:** 技术方案-M5-Two-Round检索_260422.md
**测试文件:** `test/test_m5_two_round.py`
...
| 测试目标 | 文件 | 类型 |
|---------|------|------|
| `_do_search_and_notify` 重构：0 结果短路 + fire_score 返回 | `_orch_parsers.py` | pytest |
| `score_done` handler：scoring→compressing 转换 + 降级检查 | `_orch_event.py` | pytest |
| `o_done + compressing` handler 在 catch-all 之前匹配 | `_orch_event.py` | pytest |
| `_parse_scores` 评分解析（正常 + 异常） | `_orch_parsers.py` | pytest |
| `_format_scored_rules_for_compress` 包含完整内容 | `_orch_parsers.py` | pytest |
| Prompt 模板格式化 | `_orch_prompts.py` | pytest |
| config 常量 | `config.py` | pytest |
...
from aristotle_mcp._orch_state import _save_workflow, _load_workflow
from aristotle_mcp._tools_rules import write_rule, stage_rule, commit_rule, list_rules, init_repo_tool
from aristotle_mcp.config import (
    SCORING_TOP_N, SCORE_PARALLEL_MAX,
    COMPRESS_TOP_N, COMPRESS_MAX_CHARS, COMPRESS_RULE_MAX_CHARS,
)
```
...
### TC-M5-01: 零候选短路到 done

**验证项:** V1
**目的:** list_rules 返回 0 结果时，不进入 scoring phase，直接返回 notify
...
### TC-M5-03: score_done 触发 scoring→compressing 转换
...
### TC-M5-04: o_done + compressing 返回压缩结果
...
### TC-M5-05b: _format_scored_rules_for_compress 包含完整规则内容
...
| 类别 | 数量 | 用例 |
|------|------|------|
| search + fire_score | 6 | TC-M5-01, 02 |
| score_done handler | 5 | TC-M5-03 |
| compressing handler | 2 | TC-M5-04 |
| parse_scores | 5 | TC-M5-05 |
| format + compress | 1 | TC-M5-05b |
| Prompt 模板 | 2 | TC-M5-06 |
| Config 常量 | 2 | TC-M5-07 |
| **总计** | **23** | |
...
| 测试场景 | Mock 目标 | 方式 |
|---------|----------|------|
| list_rules 返回空/指定结果 | `aristotle_mcp._tools_rules.list_rules` | `unittest.mock.patch` |
| 规则文件读取 | 真实 tmp_repo 文件 | `write_rule` + `commit_rule` |
| subagent 返回 | `_fire_r_done_event` / `_fire_c_done_event` | 直接调用 |
...
```bash
pytest test/test_m5_two_round.py -v
```

> **回归测试**: 每次修改后应运行全量回归 `pytest test/ -v`，确保 M5 改动不影响现有功能。
