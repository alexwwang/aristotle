...
|---|--------------|-------------|--------------------------|------------|
| 1 | MCP tool count = 27 after merge (25 Python MCP + 2 TS Plugin) | AC-4 | Server tool registration, 25 Python MCP + 2 TS Plugin | tool count assertion, all tools callable |
...
|---|----------------------------|--------|------------|
| 1 | Tool count constant | MCP_TOOL_COUNT_POST_MERGE constant | constant value matches 27 (25 Python MCP + 2 TS Plugin). Note: MCP_TOOL_COUNT_POST_MERGE=25 in Phase 1 spec was a pre-implementation estimate. Actual post-merge count is 27 (25 Python MCP + 2 TS Plugin). Phase 4 test will verify exact count at execution time. |
...
|---|----------|----|-----------|-----------|-----------|-------------|
| 1 | Core | AC-1 | Integration | test_integration.py | `should_perform_full_lifecycle_e2e_flow` | create_rollback_point → write_ki_doc (entry_type='intervention') → read_ki_docs (verify entry written) → commit_rule with guard (enable_guard=True) → rollback_to_checkpoint (verify pipeline_reset_required=true) → cleanup_rollback_stashes |
...
| 4 | Core | AC-1 | Unit | test_integration.py | `should_import_migrated_type_definitions` | All types from deleted intervention_types.py importable from `aristotle_mcp._types` (or equivalent migration target). **Phase 4 resolution required**: exact import path must be confirmed during Phase 4 implementation — the `_types` module may not exist yet. Test should use a configurable import path or be updated once migration target is known. **Complete type list** (from `intervention_types.__all__`): ViolationEvent, InterventionPlan, RollbackResult, CommitResult, PatternMatch, ValidationResult, TestResult, PipelineContext, InterventionResult (9 dataclasses) + BEHAVIORAL_VIOLATIONS, VIOLATION_PRIORITY (2 constants) = 11 total exports. |
...
| 7.1 | Core | AC-3 | Integration | test_integration.py | `should_verify_no_artifact_files_after_cleanup` | After rollback + cleanup, verify no stray stash entries, no temp files, no partial KI docs remain in working directory |
| 8 | Core | AC-4 | Integration | test_integration.py | `should_assert_python_mcp_tool_count_equals_25` | Assert Python MCP server tool count equals 25 (20 existing + 5 new) via `mcp._tool_manager._tools`. Full runtime tool count (including TS Plugin) = 27 (verified by constant MCP_TOOL_COUNT_POST_MERGE in Design Coverage Matrix test #1). |
| 9 | Core | AC-4 | Integration | test_integration.py | `should_register_python_mcp_tools_after_init_repo` | All 25 Python MCP tools available after init_repo. TS Plugin tools (aristotle_abort, aristotle_fire_o) are registered separately by the TypeScript plugin and verified via runtime integration (see test #8 note on MCP_TOOL_COUNT_POST_MERGE=27). |
...
| 11 | Core | AC-5 | Unit | test_integration.py | `should_verify_no_session_state_dependency_in_tools` | Tools don't store session_id across calls. Stateless verification per tool: create_rollback_point (no session state), rollback_to_checkpoint (reads from git stash, no session state), cleanup_rollback_stashes (reads stash list, no session state), write_ki_doc (writes to filesystem, no session state), read_ki_docs (reads from filesystem, no session state). Pipeline-reset tools (pipeline_reset, force_resolve_violation, resolve_timeout) are verified stateless via pipeline-reset.md tests #1–17 — no session state dependency in any reset/resolution path. |
...
| 17.1 | Core | AC-1 | Integration | test_integration.py | `should_reject_missing_required_fields_per_entry_type` | write_ki_doc rejects entries with missing required fields per entry_type: intervention requires (violation, timestamp, file, phase), assessment requires (phase, next_phase, status), merge requires (violations, phase, requirement). Verify each entry_type independently. |
...
| 20 | Core | AC-1, AC-9 | Integration | test_integration.py | `should_write_audit_entries_for_entire_lifecycle` | after E2E flow (test #1), verify audit.jsonl contains entries for each tool call in chronological order. Validate McpAuditEntry schema compliance (field types, required fields), content correctness (tool name matches operation, params match call args), truncation behavior (error field ≤ 500 chars, measured in Unicode code points — include test data with emoji/CJK characters to verify code-point-based truncation). Verify truncated=true boolean field is set when truncation occurs. **Order verification**: entries must appear in same sequence as tool calls (create_rollback_point → write_ki_doc → read_ki_docs → commit_rule → rollback_to_checkpoint → cleanup_rollback_stashes). **AC trace**: satisfies AC-9 (audit logging completeness) and AC-1 (tool integration). **Cross-module schema consistency**: verify all audit entries across tools (rollback, ki-doc, commit-guard, pipeline-reset) share identical McpAuditEntry field structure — same required fields (timestamp, tool, params, result, runId), same optional fields (error, truncated), same field types. This validates that module-level audit tests (#23-24 in rollback-tools, #16-17 in ki-doc-tools, #20-22 in commit-guard, #15-17 in pipeline-reset) produce schema-consistent entries. |
...
| 22 | High | AC-2 | Integration | test_integration.py | ~~**DEFERRED**~~: `test_preservation_under_migration` — to be implemented when Phase 4 migration code is available. Tracked in known-issues.md. Row retained for traceability; no test implementation in this phase. **Relationship to test #2**: Test #2 covers AC-2 by running the existing test suite post-migration (execution gate). Test #22 is a supplementary fine-grained preservation test for individual migration artifacts. Test #2 alone satisfies AC-2; test #22 adds depth but is not required for the gate. |
| 23 | High | AC-7 | Integration | test_integration.py | `should_verify_commit_guard_blocks_invalid_rule_in_e2e` | End-to-end AC-7 integration test: write_rule → stage_rule → commit_rule with enable_guard=True (missing category) → verify guard blocks commit → verify McpAuditEntry written with result='error'. Validates guard integration in full tool chain. |
| 24 | High | AC-1, AC-10 | Integration | test_integration.py | `should_rollback_then_pipeline_reset_state_chain` | Integration test verifying rollback→pipeline_reset state reset chain: create_rollback_point → modify state → rollback_to_checkpoint (returns pipeline_reset_required=true) → pipeline_reset (resets state). Verify state fields (observerTimeoutCount, auditEntryCount, phase) are clean after chain completes. |

## Design Coverage Matrix (Phase 2 → Tests)

| # | Priority | Design Element | Element Type | Test Type | Test File | Test Name | Description |
|---|----------|---------------|-------------|-----------|-----------|-----------|-------------|
| 1 | Key | MCP_TOOL_COUNT_POST_MERGE constant | Constant | Unit | test_integration.py | `should_assert_mcp_tool_count_equals_27` | Constant value equals 27 (25 Python MCP + 2 TS Plugin) |
...
- **Tool list**: 27 tool names for count assertion (25 Python MCP + 2 TS Plugin). Note: MCP_TOOL_COUNT_POST_MERGE=25 in the design spec was a pre-implementation estimate; actual post-merge count is 27.
...
- **AC-2 scope**: Existing test suites in scope: aristotle_mcp/tests/test_mcp_server_tools.py (20 existing tool tests). Phase 4 will add: test_mcp_ki_doc.py, test_commit_guard.py, test_pipeline_reset.py, test_mcp_audit_log.py, test_rollback_tools.py, test_integration.py. AC-2 execution gate runs ALL tests in aristotle_mcp/tests/ post-migration.
...
- **Bilingual forbidden patterns for AC-6** (frozen list — do not modify without updating this test):
  - English patterns: ["total N issues so far", "Round N of M", "all issues resolved", "you may stop"]
  - Chinese patterns: ["累计N个问题", "第N轮", "已全部修复", "可提前结束"]
  - **Note**: The frozen list above is representative of the patterns expected in the Reviewer prompt — additional patterns may be discovered during Phase 4 implementation. Test #12 verifies the frozen list above exists in the Reviewer prompt. Test #21 verifies 1:1 migration of the **original PromptValidator source patterns** (the full set from `intervention/src/prompt_validator.py`), not just the representative subset listed here. The "representative" note applies to the frozen test data list only; test #21's "1:1 mapping" assertion applies to the complete original source pattern set.
