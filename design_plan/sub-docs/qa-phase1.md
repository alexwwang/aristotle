...
- **CheckpointGateResult** → 完整定义见 §3.0.1
- **appendAudit** → 完整签名与行为见 §3.0.2
- **getUnresolvedViolations** → 完整签名与行为见 §3.0.2
 - **PipelineStore 新增方法**：`getUnresolvedViolations` 和 `resolveViolations` → 完整签名与行为规范见 §3.0.2
...
- **AuditLogEntry.event 类型扩展**：
  ...
  - **CheckpointEvent Phase 1 扩展** — 完整 payload/precondition/action 定义及 applyTransition case → 见 §3.0.1 CheckpointEvent。applyTransition 实现约束 → 见 §3.0.1。§3.1.2 保留以下设计上下文：
  - 若不添加 validateTransition case，transitions.ts default 分支会拒绝这些事件（`fail('Unknown event')`）。
  ...
  - Observer/审计生成的事件 string 值统一使用 SCREAMING_SNAKE_CASE 命名。CheckpointEvent 值保持 lowercase_with_underscore（历史遗留，如 pipeline_start、phase_complete）。
- **AuditLogEntry** → 完整接口定义（含 event 联合类型、Migration Note、差异说明）见 §3.0.1
- **新增运行时依赖**（Phase 1）：
...
| 3 | 误拦截率 | ≤5%（误拦截率 = 被错误阻止的合法操作数 / 总合法操作数 × 100%） | e2e 测试：执行 50 个合法操作，统计误拦截。**测试环境配置**：ignoreExitCodes=[1, 130]，ignoreCommands=["git log *", "man *"]。（合法操作白名单：写入有效 JSON 文件、执行 exit 0 的 Bash 命令、执行 `git status` 等只读命令、执行预期非零退出码的命令（grep 无匹配返回 1、diff 有差异返回 1、test 条件不满足返回 1）——此类命令通过 ignoreExitCodes 配置排除）。⚠️ Phase 1 不验证 .ts/.tsx 文件（§3.1.1 extensions 默认值），白名单不包含 .ts 文件写入） |
