# Pseudocode Reference — §3.2 Phase 2 Test Gate
...
## Code Block 1: L856-951
**Context**: **不在 Interceptor 或 Observer 中运行测试**。改为 Checkpoint 审计日志模式：
...
## Code Block 4: L1003-1028
**Context**: 6. **运行时检测**：Watchdog 初始化时尝试注册自定义工具，若 API 不可用则自动切换至降级模式（无需手动配置）。降级检测逻辑详见 §4.2 OBSERVER_TIMEOUT 降级机制，实现时应提取为独立私有方法 `_checkDegradation(state, sessionID)`。伪代码如下：
