...
import { extractLastAssistantText } from '@opencode-ai/core/utils';
import type { WorkflowState } from '@opencode-ai/core/types';
import type { WorkflowStore } from '@opencode-ai/core/store/workflow-store';
import type { AristotleExecutor } from './executor.js';
import { createLogger } from '@opencode-ai/core/logger';

const logger = createLogger('aristotle', 'ARISTOTLE_LOG');
...
export class IdleEventHandler {
  ...
  constructor(
    ...
    private executor: AristotleExecutor,
    options: { sessionsDir: string; mcpDir: string },
  ) {
  ...
  private async driveChainTransition(
    ...
    if (action.action === 'fire_sub') {
      ...
      if (!action.sub_prompt) {
        ...
        logger.error('missing sub_prompt: wf=%s', action.workflow_id);
        return;
      }
      try {
