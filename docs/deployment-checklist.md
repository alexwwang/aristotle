...
## 2. MCP 安装 (~/.claude/skills/aristotle/)

| # | 检查项 | 验证命令 | 预期结果 |
|---|--------|----------|----------|
| 2.1 | 源文件同步 | `diff <(ls aristotle_mcp/) <(ls ~/.claude/skills/aristotle/aristotle_mcp/)` | 无差异 |
| 2.2 | _cli.py 一致 | `diff aristotle_mcp/_cli.py ~/.claude/skills/aristotle/aristotle_mcp/_cli.py` | 无差异 |
| 2.3 | _orch_start.py 一致 | `diff aristotle_mcp/_orch_start.py ~/.claude/skills/aristotle/aristotle_mcp/_orch_start.py` | 无差异 |
| 2.4 | _orch_event.py 一致 | `diff aristotle_mcp/_orch_event.py ~/.claude/skills/aristotle/aristotle_mcp/_orch_event.py` | 无差异 |
| 2.5 | venv 存在 | `ls ~/.claude/skills/aristotle/.venv/bin/python` | 文件存在 |
| 2.6 | 模块可导入 | `cd ~/.claude/skills/aristotle && uv run python -c "from aristotle_mcp.server import mcp; print('OK')"` | 输出 OK |
| 2.7 | _cli.py 支持 orchestrate_start | `grep 'orchestrate_start' ~/.claude/skills/aristotle/aristotle_mcp/_cli.py` | 有匹配 |
...
| # | 检查项 | 验证命令 | 预期结果 |
|---|--------|----------|----------|
| 4.1 | 文件存在 | `ls ~/.claude/skills/aristotle/SKILL.md` | 文件存在 |
| 4.2 | 与源码一致 | `diff SKILL.md ~/.claude/skills/aristotle/SKILL.md` | 无差异 |
| 4.3 | 无 agent 参数引用 | `grep 'agent=response.sub_role' ~/.claude/skills/aristotle/SKILL.md` | 无匹配 |
| 4.4 | 包含 Bridge 路径 | `grep 'aristotle_fire_o' ~/.claude/skills/aristotle/SKILL.md` | 有匹配 |
...
check "2.2 _cli.py sync"       "diff aristotle_mcp/_cli.py ~/.claude/skills/aristotle/aristotle_mcp/_cli.py"
check "2.3 _orch_start sync"   "diff aristotle_mcp/_orch_start.py ~/.claude/skills/aristotle/aristotle_mcp/_orch_start.py"
check "2.6 module importable"  "cd ~/.claude/skills/aristotle && uv run python -c \"from aristotle_mcp.server import mcp\""
check "3.4 permission set"     "grep aristotle_\\* ~/.config/opencode/opencode.json"
check "4.2 SKILL.md sync"      "diff SKILL.md ~/.claude/skills/aristotle/SKILL.md"
check "4.3 no agent param"     "test $(grep -c agent=response.sub_role ~/.claude/skills/aristotle/SKILL.md) -eq 0"
...
# 3. 同步 MCP
rsync -av --exclude='.venv' --exclude='.git' --exclude='__pycache__' --exclude='node_modules' $ARISTOTLE_PROJECT_DIR/aristotle_mcp/ ~/.claude/skills/aristotle/aristotle_mcp/

# 4. 同步 SKILL.md
cp $ARISTOTLE_PROJECT_DIR/SKILL.md ~/.claude/skills/aristotle/SKILL.md
