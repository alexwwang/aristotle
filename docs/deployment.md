...
| # | 检查项 | 验证命令 | 预期结果 |
|---|--------|----------|----------|
| 1.1 | 部署文件存在 | `ls ~/.config/opencode/aristotle-plugin/index.js` | 文件存在 |
| 1.2 | 大小匹配源码构建 | `wc -c ~/.config/opencode/aristotle-plugin/index.js` vs `wc -c plugin/dist/index.js` | 字节数一致 |
| 1.3 | 内容一致 | `diff ~/.config/opencode/aristotle-plugin/index.js plugin/dist/index.js` | 无差异 |
| 1.4 | 使用 spawn（非 execFile） | `grep -c spawn ~/.config/opencode/aristotle-plugin/index.js` | ≥1 |
| 1.5 | 无 execFile/promisify | `grep -c 'execFile\|promisify' ~/.config/opencode/aristotle-plugin/index.js` | 0 |
| 1.6 | promptAsync 不传 agent | `grep -c 'agent, parts' ~/.config/opencode/aristotle-plugin/index.js` | 0 |
| 1.7 | trigger 用 session_id 作 parent | `grep 'trigger.session_id' ~/.config/opencode/aristotle-plugin/index.js` | 有匹配 |
...
check "1.2 plugin size match"  "diff ~/.config/opencode/aristotle-plugin/index.js plugin/dist/index.js"
check "1.5 no execFile"        "test $(grep -c execFile ~/.config/opencode/aristotle-plugin/index.js) -eq 0"
check "1.6 no agent in prompt" "test $(grep -c \"agent, parts\" ~/.config/opencode/aristotle-plugin/index.js) -eq 0"
...
```bash
# 1. 构建插件
cd $ARISTOTLE_PROJECT_DIR/plugin && bun build index.ts --outdir dist --target node --format esm --external @opencode-ai/plugin

# 2. 部署插件
cp plugin/dist/index.js ~/.config/opencode/aristotle-plugin/index.js
