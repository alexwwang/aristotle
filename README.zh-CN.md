...
> **注意：** 此方式只有基础 skill，不含 MCP server。你将无法获得 Git 版本控制、Δ 审核决策和规则状态管理。需执行 `uv sync` 并添加 MCP 配置（见下方）才能启用完整功能。learnings 文件会在首次运行时自动创建。
