...
class TestIntegration:
    ...
    def test_should_verify_intervention_directory_deleted(self) -> None:
        ...
        assert not intervention_dir.exists(), f"intervention/ directory still exists at {intervention_dir}"
    ...
    # ---------------------------------------------------------------
    def test_should_execute_tools_without_prior_session_state(self, tmp_repo: Path) -> None:
    ...
    # ---------------------------------------------------------------
    def test_should_verify_no_session_state_dependency_in_tools(self, tmp_repo: Path) -> None:
    ...
    def test_should_verify_bilingual_patterns_in_ralph_loop_reviewer(self) -> None:
        ...
        for py_file in patterns_dir.rglob("*.py"):
            ...
            if any(p in content for p in ["total N issues so far", "Round N of M", "issues so far"]):
                found_en = True
    ...
    def test_should_verify_all_preserved_modules_have_interface_docs(self) -> None:
        ...
        preserved_public = [
            name for name in dir(server_mod) if name.startswith("aristotle_") and callable(getattr(server_mod, name))
        ]
    ...
    def test_should_verify_no_stale_imports_in_existing_tests(self) -> None:
        ...
        for tf in test_dir.glob("*.py"):
            ...
            assert "import intervention" not in content, f"Stale 'import intervention' found in {tf}"
            assert "from intervention" not in content, f"Stale 'from intervention' found in {tf}"
    ...
    def test_should_verify_no_stale_imports_in_preserved_modules(self) -> None:
        ...
        for py_file in src_dir.rglob("*.py"):
            if py_file.name == "test_integration.py":
                continue
            content = py_file.read_text(errors="ignore")
            assert "from intervention" not in content, f"Stale 'from intervention' in {py_file}"
            assert "import intervention" not in content, f"Stale 'import intervention' in {py_file}"

    # ---------------------------------------------------------------
    # 20. Audit entries for entire lifecycle
    # ---------------------------------------------------------------
    def test_should_write_audit_entries_for_entire_lifecycle(self, tmp_repo: Path) -> None:
    ...
    def test_should_verify_all_original_prompt_validator_patterns_migrated(
        ...
        assert migrated_patterns_file.exists(), "Migrated patterns file not found"
    ...
    def test_preservation_under_migration(self) -> None:
        ...
        for tool_name in EXISTING_TOOLS:
            assert tool_name in registered, f"Preserved tool '{tool_name}' missing from MCP registry after migration"
