...
import aristotle_mcp.intervention.compliance as compliance_mod
...
def _reset_compliance_globals():
    """Reset module-level singletons so CommitGuard state never leaks between tests."""
    compliance_mod._GLOBAL_GUARD = None
    compliance_mod._GLOBAL_COORDINATOR = None
    compliance_mod._GLOBAL_KI_DOC = None
    yield
    compliance_mod._GLOBAL_GUARD = None
    compliance_mod._GLOBAL_COORDINATOR = None
    compliance_mod._GLOBAL_KI_DOC = None
...
def test_batch_emits_all_violation_events_in_single_intervene_batch():
    CommitGuard(project_root="/tmp/test_repo")._commit_failures["INT-abc123:4"] = 3
    ctx = _make_context()
...
# C-24
def test_final_commit_performed_after_ki_doc_update(tmp_path):
    _init_git_repo(tmp_path)
    tracked = tmp_path / "tracked.txt"
    tracked.write_text("v1")
    subprocess.run(["git", "add", "."], cwd=str(tmp_path), capture_output=True, check=True)
    subprocess.run(["git", "commit", "-m", "init"], cwd=str(tmp_path), capture_output=True, check=True)
    tracked.write_text("v2")  # dirty the tree so post-batch commit has work to do

    events = [_make_violation(ViolationType.MISSING_KI_DOC)]
    result = _handle_merged(events, context=_make_context(project_root=str(tmp_path)))
